...
$login_message          = apply_filters( 'kapee_form_checkout_login_message', esc_html__( 'If you have already registered, please, enter your details in the boxes below. If you are a new customer, please, go to the Billing &amp; Shipping section.', 'woocommerce' ) );
...
<div id="multi-step-checkout" class="multi-step-checkout">
    ...
    <?php if ( $enable_login_reminder ) { ?>
        <div id="checkout_login" data-step="<?php echo esc_attr($step);?>" data-steptitle="login" class="panel panel-default <?php echo esc_attr($step == 1) ? 'active':''?> <?php echo esc_attr($step > 1) ? 'hidden':''?> woocommerce_checkout_login">
			<div class="panel-heading">
				...
				<span class="edit-action" data-step="<?php echo esc_attr($step);?>"><?php esc_html_e('Change','woocommerce');?></span>
			</div>
				   ...
				   <div class="checkout_login <?php echo esc_attr( $is_logged_in ) ? 'logged-in' : 'not-logged-in';?>">
						<?php if($is_logged_in) {
							...
							<div class="logged-in-user-info">
								<div class="user-info">
									<span><?php esc_html_e('Name','woocommerce');?></span>
							...
							<div class="checkout-next-step" data-next="<?php echo esc_attr('step-').($step+1);?>">
								<a class="btn" href="javscript:void(0);"> <?php echo esc_html_e('Continue','woocommerce');?></a>
							</div>
						<?php } else { ?>
							<?php do_action( 'kapee_checkout_login_form', $login_message );
							if( $enable_signup_and_login ){ ?>
							<div class="btn checkout-next-step" data-next="<?php echo esc_attr('step-').($step+1 );?>"><?php esc_html_e('I don\'t have an account.', 'woocommerce');?></div>
						<?php }
						} ?>
					</div>
				</div>
    ...
    if ( ! $checkout->is_registration_enabled() && $checkout->is_registration_required() && ! is_user_logged_in() ) {
        echo apply_filters( 'woocommerce_checkout_must_be_logged_in_message', esc_html__( 'You must be logged in before proceeding to checkout.', 'woocommerce' ) );
        return;
    } ?>
    ...
    <form name="checkout" method="post" class="checkout woocommerce-checkout" action="<?php echo esc_url( wc_get_checkout_url() ); ?>" enctype="multipart/form-data">
        ...
        <?php if ( sizeof( $checkout->checkout_fields ) > 0 ) { ?>
			...
			<div id="step-<?php echo esc_attr($step);?>" class="panel panel-default <?php echo esc_attr($step == 1) ? 'active':''?> <?php echo esc_attr($step > 1) ? 'hidden':''?>" data-step="<?php echo esc_attr($step);?>" data-steptitle="billing">
				<div class="panel-heading">
					<h4 class="panel-title">
						<span class="step-numner"><?php echo esc_html( $step ); ?></span>
						<span class="title"><?php esc_html_e('Billing Address','woocommerce');?></span>						
					</h4>
					<span class="edit-action" data-step="<?php echo esc_attr($step);?>"><?php esc_html_e('Change', 'woocommerce');?></span>
				</div>
				<div id="kapee-billing" class="panel-collapse collapse <?php echo esc_attr($step == 1) ? 'show':''?>">
					<div class="panel-body">
					  ...
					  <div class="btn checkout-next-step" data-next="<?php echo esc_attr('step-').($step+1);?>"><?php echo esc_html_e('Continue','woocommerce');?></div>
					</div>
				</div>
				<?php $step++;?>
			</div>
            <div id="step-<?php echo esc_attr($step);?>" class="panel panel-default <?php echo esc_attr($step == 1) ? 'active':''?> <?php echo esc_attr($step > 1) ? 'hidden':''?>" data-step="<?php echo esc_attr($step);?>" data-steptitle="shipping">
				<div class="panel-heading">
					<h4 class="panel-title">
						<span class="step-numner"><?php echo esc_html( $step ); ?></span>
						<span class="title"><?php esc_html_e('Shipping Address','woocommerce');?></span>
					</h4>
					<span class="edit-action" data-step="<?php echo esc_attr($step);?>"><?php esc_html_e('Change', 'woocommerce');?></span>
				</div>
		...
		<div id="step-<?php echo esc_attr($step);?>" class="panel panel-default <?php echo esc_attr($step == 1) ? 'active':''?> <?php echo esc_attr($step > 1) ? 'hidden':''?>" data-step="<?php echo esc_attr($step);?>" data-steptitle="payment">
			<div class="panel-heading">
				<h4 class="panel-title">
					<span class="step-numner"><?php echo esc_html( $step ); ?></span>
					<span class="title"><?php esc_html_e('Payment Options','woocommerce');?></span>
				</h4>
				<span class="edit-action pull-right" data-step="<?php echo esc_attr($step);?>"><?php esc_html_e('Change', 'woocommerce');?></span>
			</div>
